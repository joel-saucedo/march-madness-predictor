...
# Define the range of years and list to hold game data
years = range(2002, 2025)  # 2002 to 2024 inclusive
games = []
...
for year in years:
    ...
    if not switcher:
        ...
        continue
    # Extract region names from each child <div>'s <a>
    ...
    # Make sure we have exactly 5 regions; if not, skip this year
    if len(region_labels) < 5 or len(region_containers) < 5:
...
# Save the aggregated game data to a CSV file
df = pd.DataFrame(games)
output_file = os.path.join(output_dir, "ncaa_2002_2024_tournament.csv")
